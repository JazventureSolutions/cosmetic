...
class StateController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            return app('datatables')->of(State::select([
                'states.id',
                'states.name',
                'states.state_code',
            ]))
                ...
                ->addColumn('actions', function ($state) {
                    $html = '<a href="' . $state->edit_route . '" class="btn btn-sm btn-default btn-text-primary btn-hover-primary btn-icon mr-2" title="Edit details">
        ...
        return view('settings.setups.states.list', [
            'title' => $title
        ]);
    }
    ...
    public function edit($id = 0)
    {
        ...
        if (Route::currentRouteName() == 'settings.setups.states.edit' || Route::currentRouteName() == 'settings.setups.states.profile') {
            $state = State::findOrFail($id);
        }

        return view('settings.setups.states.edit', [
            'title' => $title,
            'mode' => $mode,
            'user' => $state
        ]);
    ...
    public function store(Request $request)
    {
        ...
        switch ($mode) {
            case 'profile':
                ...
                if (!$isEdit || $hasPassword) {
                    if (!Hash::check($request->old_password, $state->password)) {
                ...
                $data = [
                    ...
                    'email' => $request->email ?? $state->email,
                    'phone' => $request->phone,
                ];
